...
def main():
    ...
    for trigger in z.trigger.get(
            output=['triggerid', 'description', 'priority', 'lastchange'],
            groupids=34,
            filter={"value": 1},
            sortfield='lastchange'):
        if trigger[
                'description'] == 'Zabbix agent on {HOST.NAME} is unreachable':
            trigmsg = z.trigger.get(triggerids=trigger['triggerid'],
                                    selectHosts='extend')
            lastchange = datetime.datetime.fromtimestamp(
